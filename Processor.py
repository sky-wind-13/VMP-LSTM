...
class Processor():
    def __init__(self, args):
        ...
        self.Trajectory = open(os.path.join(self.args.model_dir, 'Trajectory.txt'), 'a+')
    ...
    def playtrain(self):
        ...
        val_error,val_final=0,0
        best_epoch = 0
        min_tarin_error = float('inf')
        min_loss_count = 0
        for epoch in range(self.args.num_epochs):
            ...
            #log files
            # self.log_file_curve.write(str(epoch) + ',' + str(train_loss) + ',' 
            #                           +  ','+str(test_error) + ',' + str(test_final_error) + '\n')
            ...
            #console log
            print('----epoch {}, train_loss={:.8f}, valid_error={:.8f}, valid_final={:.8f},test_error={:.8f},test_final_error={:.8f}'
                  .format(epoch, train_loss,val_error, val_final,test_error,test_final_error))
            # print('----epoch {}, train_loss={:.5f}, test_error={:.3f},test_final_error={:.3f}'
            #       .format(epoch, train_loss,test_error,test_final_error))
            
            
            # 跟踪最小损失
            if train_loss < min_tarin_error:
                min_tarin_error = train_loss
                min_loss_count = 0
                best_epoch = epoch
            else:
                min_loss_count += 1
                if min_loss_count >= 20:
                    print('训练停止。连续20轮中最小损失未减少。')
                    break
            
            print('----best-epoch {}, min_tarin_error={:.5f}'
                  .format(best_epoch, min_tarin_error))
    ...
    def train_epoch(self,epoch):
        ...
        for batch in range(self.dataloader.trainbatchnums):
            ...
            # print(np.array_equal(batch_abs,batch_norm))
            # print('batch_abs',batch_abs)
            ...
            # 写入预测值与目标值
            #self.Trajectory.write(str(epoch) + ',' + str(outputs)+','+str( batch_norm[1:,:,:2])+'\n')
            ...
            if batch%self.args.show_step==0 and self.args.ifshow_detail:
                ...
                'train-{}/{} (epoch {}), train_loss = {:.5f}, time/batch = {:.5f}'.format(batch,self.dataloader.trainbatchnums,
                                                                                epoch,
                                                                                loss.item(), end - start))
                # print('细节',outputs,batch_norm[1:,:,:2])
                self.Trajectory.write('batch'+str(batch) + 'trainchnums'+str(self.dataloader.trainbatchnums)+'epoch'+str(epoch) + 'output' + str(outputs)+' batch_norm'+str( batch_norm[1:,:,:2])+'\n')
    ...
    def val_epoch(self, epoch):
        ...
        for batch in range(self.dataloader.valbatchnums):
            ...
            outputs_infer, _, _, look = forward(inputs_fw, iftest=False)
            ...
            if batch%self.args.show_step==0 and self.args.ifshow_detail:
                print(
                'val-{}/{} , val_error = {:.5f}, time/batch = {:.5f}, error_epoch={:.5f},error_cnt_epoch={:.5f},final_error_epoch={:.5f}'.format(batch,self.dataloader.valbatchnums,
                                                                                final_error_cnt_epoch, end - start,error_epoch,error_cnt_epoch,final_error_epoch))
    ...
    def test_epoch(self,epoch):
        ...
        for batch in range(self.dataloader.testbatchnums):
            ...
            # outputs_infer, _, _, look = forward(inputs_fw, iftest=True)
            # print(np.array_equal(batch_abs,batch_norm))
            # print('inputs_fw',batch_norm)
            # 打开文件，如果文件不存在则创建
            
            outputs_infer, _, _, look = forward(inputs_fw, iftest=False)
            # with open('outputs_infer.txt', 'w') as file:
            #     # 遍历元组中的每个元素
            #     for element in outputs_infer:
            #         # file.write(str(element) + '\n\n')
            #         # 将每个元素转换为字符串并写入文件，后面跟一个换行符
            #         file.write(str(element) + '\n\n')
            # numpy_arrays = [tensor.cpu().numpy() for tensor in outputs_infer]
            numpy_arrays = outputs_infer.cpu().detach().numpy()
            
            # csvfile = 'outputs_infer.csv'
            # 使用Pandas将数组保存到CSV文件中
            with open('outputs_infer.csv', 'w') as csvfile:
                csvwriter = csv.writer(csvfile)
                for array in numpy_arrays:
                    csvwriter.writerows(array)

            # 或者使用Pandas的to_csv方法
            # 将所有数组合并为一个大的DataFrame
            combined_df = pd.concat([pd.DataFrame(arr) for arr in numpy_arrays], ignore_index=True)
            combined_df.to_csv('outputs_infer.csv', index=False)
            # print('outputs_infer',len(outputs_infer))
            # print('output',outputs_infer)
            ...
            lossmask, num = getLossMask(outputs_infer, seq_list[0], seq_list[1:], using_cuda=self.args.using_cuda)
            # 将每个张量转移到CPU并转换为NumPy数组
            numpy_arrays = [tensor.cpu().detach().numpy() for tensor in batch_norm]

            # 使用Pandas将数组保存到CSV文件中
            combined_df = pd.concat([pd.DataFrame(arr) for arr in numpy_arrays], ignore_index=True)
            combined_df.to_csv('batch_norm.csv', index=False)
