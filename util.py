...
class DataLoader_bytrajec2():
    def __init__(self, args):
        ...
        self.args=args
        if self.args.dataset=='shanghai':
            
            
            # self.data_dirs = ['./taiwan/train/','./taiwan/test/']
            self.data_dirs = ['./data/eth/univ', './shanghai/train/',
                              './taiwan/train/',
                              './taiwan/test/', './taiwan/val/',
                              ]
            ...
            '''
            # 假设 args 是一个已解析的命令行参数对象  
            skip = [ 10, 10]   # 注意：这个列表可能需要根据实际情况进行调整  
            ...
            # 生成 train_set，包含所有数据的索引  
            train_set = list(range(len(self.data_dirs)))  

            # 根据 args.test_set 设置 test_set  
            if isinstance(args.test_set, int):  
                self.test_set = [args.test_set]  
            elif isinstance(args.test_set, list) and all(isinstance(x, int) for x in args.test_set):  
                self.test_set = args.test_set  
            else:  
                raise ValueError("args.test_set must be an integer or a list of integers")  

            # 从 train_set 中移除 test_set 中的元素  
            train_set = [x for x in train_set if x not in self.test_set]  

            # 更新目录和跳过信息  
            self.train_dir = [self.data_dirs[x] for x in train_set]  
            self.test_dir = [self.data_dirs[x] for x in self.test_set]  

            # 生成 trainskip 和 testskip  
            # 注意：这里假设 skip 列表的长度足够长，或者我们有一个方法来处理索引超出范围的情况  
            # 如果 skip 的长度可能不够，你需要添加一些错误处理或逻辑来确保不会访问不存在的索引  
            self.trainskip = [skip[x] if x < len(skip) else None for x in train_set]  # 防止索引超出范围  
            self.testskip = [skip[x] if x < len(skip) and x in self.test_set else None for x in range(len(self.test_set))]  # 但这里其实不需要检查 x in self.test_set，因为 x 直接来自 self.test_set  
            # 或者更简单地，因为 self.test_set 中的元素已经是索引  
            self.testskip = [skip[x] if x < len(skip) else None for x in self.test_set]
    ...
    def get_seq_from_index_balance(self,frameped_dict,pedtraject_dict,data_index,setname):
        ...
        for i in range(data_index.shape[1]):
            ...
            # print("batch_pednum:",batch_pednum)
    ...
    def rotate_shift_batch(self,batch_data,ifrotate=True):
        ...
        return batch_data
        # batch_data=batch,batch,0,seq_list,nei_list,nei_num,batch_pednum
        # return batch_data
